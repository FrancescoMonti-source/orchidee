...
For a routine build, the boundary is:

| You provide | Already in the checkout | Generated by ORCHIDEE |
| --- | --- | --- |
| BACT path, PMSI path | Rouen references, mappings and analytical rules | Six handoff blocks, validated bundles, audit and manifest |

On a fresh clone, restore the locked R environment once:

```powershell
& .\scripts\setup.ps1
```

Retrieve the automatic BACT export and the PMSI object produced by `redsan`.
They may stay in a protected location outside the repository. Check the
environment and both paths without reading the clinical objects:
...
Remove `-DryRun` to start the build. The default output is
`outputs/rouen_current`. Add
...
accepted. A filesystem root or a directory containing the checkout is
rejected. A successful run ends with `PASS` and writes
`<Output>\build_manifest.txt` (`outputs/rouen_current/build_manifest.txt` by
default). The manifest is the completion marker; if it is absent, do not use
the output. Expect minutes rather than seconds: the current roughly 50 MB
Rouen exports took about 19 minutes on the qualification workstation, with
little console output during the calculation. Do not prepare any intermediate
file manually.
