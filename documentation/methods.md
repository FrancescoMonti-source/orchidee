...
| Pour un site, l'unité d'un prélèvement est l'unité d'hébergement active à l'heure du prélèvement, dérivée de ses intervalles d'hospitalisation ; le site n'en fournit pas. Un prélèvement non situable donne `NA`, donc sort du périmètre au lieu d'être rattaché au hasard. L'attribution appliquée est celle de la ligne précédente, sans seconde implémentation | `R/site_handoff_preparation_helpers.R` → `orchidee_site_prepare_handoff()` | Quels prélèvements entrent au numérateur, pour tout établissement sans adaptateur |
...
| `EVTID` est obligatoire : la colonne doit exister et toute ligne sans identifiant de séjour est écartée avant propagation du dépistage. L'identité est toujours `PATID + EVTID + ELTID`, sans repli sur le patient seul. Un fichier sans séjour utilisable ne peut pas produire de bundle ; un bundle matérialisé ne peut contenir d'`EVTID` absent ou vide | `R/external_handoff_helpers.R` → `orchidee_handoff_document_occurrence_key()`, `orchidee_handoff_build_sir_wide_from_microbiology()` ; `R/rouen_microbiology_handoff_adapter.R` → `build_rouen_microbiology_handoff()` ; `R/external_bundle_validation_helpers.R` → `external_bundle_validate_sir_wide()` | Population analytique. Les exclusions sont comptées dans les diagnostics site et l'audit Rouen `rows_dropped_without_evtid`. Un marqueur de dépistage porté par une ligne écartée disparaît avec elle |
...
| Décision | Implémentation | Effet |
|------------------------|------------------------|------------------------|
| La période d'analyse d'un site est déclarée explicitement, en années civiles consécutives : elle sélectionne la microbiologie et découpe l'exposition | `R/site_handoff_preparation_helpers.R` → `orchidee_site_resolve_period()` | Les lignes analysées d'un établissement sans adaptateur |
| Cette période voyage jusqu'au rendu par l'environnement du processus, et non par une modification du fichier de configuration partagé : un rendu Rouen ordinaire garde la valeur déclarée | `config/pipeline.R` → `orchidee_report_years_from_environment()` | Les années publiées, par rendu |
