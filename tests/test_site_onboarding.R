...
if (identical(.Platform$OS.type, "windows")) {
  ...
  wrapper_template_output <- suppressWarnings(system2(
    powershell,
    c(
      "-NoProfile",
      "-ExecutionPolicy",
      ...
      "-EmitTemplates",
      shQuote(wrapper_template_dir)
    ),
    stdout = TRUE,
    stderr = TRUE
  ))
  wrapper_template_status <- attr(wrapper_template_output, "status")
  if (is.null(wrapper_template_status)) wrapper_template_status <- 0L
  wrapper_template_run <- list(
    status = wrapper_template_status,
    output = wrapper_template_output
  )
  ...
  run_site_wrapper <- function(
    ...
    wrapper_args <- c(
      ...
      shQuote(input_paths[[1L]]),
      "-BacteriaMapping",
      shQuote(input_paths[[2L]]),
      "-SampleTypeMapping",
      shQuote(input_paths[[3L]]),
      "-AntibioticMapping",
      shQuote(input_paths[[4L]]),
      "-UnitMapping",
      shQuote(input_paths[[5L]]),
      "-IncidenceExposure",
      shQuote(input_paths[[6L]]),
      "-Output",
      shQuote(output_path)
    )
    if (dry_run) wrapper_args <- c(wrapper_args, "-DryRun")
  ...
  pwsh <- Sys.which("pwsh.exe")
  if (nzchar(pwsh)) {
    wrapper_pwsh_dry_run <- run_site_wrapper(shell = pwsh)
  }
}
...
stopifnot(
  ...
  all(grepl("scripts\\\\run_r.ps1", help_text)),
...
if (identical(.Platform$OS.type, "windows")) {
  stopifnot(
    ...
    {
      expected_bundle_literal <- paste0(
        "'",
        gsub(
          "'",
          "''",
          normalizePath(
            file.path(build_output_dir, "bundle_v2_operational"),
            winslash = "\\",
            mustWork = TRUE
          ),
          fixed = TRUE
      ...
      any(grepl(
        paste0("-Bundle ", expected_bundle_literal),
        wrapper_build_run$output,
        fixed = TRUE
      ))
    },
...
cat("PASS: site onboarding contract, templates, help, DryRun and build\n")
